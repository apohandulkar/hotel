...
#accordion .panel-body p {
  ...
  <a href="<?php echo site_url();?>waiter/waiter/cash" class="btn btn-dblue" role="button">Cash 
                          ...
                          <a href="<?php echo site_url();?>waiter/waiter/Reserve" class="btn btn-dblue active" role="button">Reserve
						   <span style="background-color:#FFA533;color:#fff;padding:0 2px 0 2px;font-size: 10px;"><?=$dcountareserve?></span></a>
...
<!------ Include the above in your HEAD tag ---------->
			...
			<div class="row">				
				<div class="col-md-12">
					...
					?>
						<div class="col-md-12" id="completecell<?=$r['id']?>">
						<div class="panel panel-default">
							<div class="panel-heading" role="tab" id="headingOne_<?=$id?>">
								<h4 class="panel-title">
									...
									#<?=$r['id']?><br />
										<font color="#fff" size="3px"> No of People | <?=$r['no_of_people'] ?> 
										<!--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;VIEW CART <i class="fa fa-briefcase" aria-hidden="true"></i>-->
										<br />Mobile : <?=$r['mobile']?> <br>
                    <div id="buttondiv<?=$r['id']?>">
                    <?php if($r['order_status']=='Waiting'){ ?>
                      <input type="button" data-id="<?=$r['id']?>" data-value="Accept" value="Accept" class="pull-right responsebtn" style="color: green;margin-top: -30px;margin-right: 75px;">
                    <input type="button" data-id="<?=$r['id']?>" data-value="Deny" value="Deny" class="pull-right responsebtn" style="color:green;margin-top: -30px;">
                     <?php }else{ ?>
                    <p id="demo" class="pull-right" style="text-align: center;font-size: 23px; margin-top: -30px;margin-right: 75px;"><?=$r['reserve_timing']?></p>
                    <img class="pull-right" src="/KOT/logo/check.png" onclick="completereserve(<?=$r['id']?>)" style="margin-top: -36px;width: 30px;margin-right: 12px;">
                      <?php } ?>
                    </div>
                    <p class="custid" data-id="<?=$r['id']?>" data-status="<?=$r['order_status']?>" data-time="<?=$r['reserve_timing']?>"></p>
										</font>
								</a>
...
<script>
  console.log("<?php echo strtotime(date('Y-m-d H:i:s')); ?>")
/*
Author       : Theme_ocean.
...
$(".responsebtn").click(function(){
      ...
      $.ajax({
        url : "/KOT/waiter/Waiter/UpdateReserveTableStatus",
        method : "POST",
        data : {id : id, status: status},
        success :function(data){
         if (status=='Accept') {
          var str=` <p id="demo" class="pull-right" style="text-align: center;font-size: 23px; margin-top: -30px;margin-right: 75px;">`+data.reserve_timing+`</p>
                    <img class="pull-right" src="/KOT/logo/check.png" onclick="completereserve(`+id+`)" style="margin-top: -36px;width: 30px;margin-right: 12px;">`;
            $("#buttondiv"+id).html(str);        
         }else{
          $("#completecell"+id).remove();
         }
        }
      })
})
...
function displayHello() {
  ...
  if (cusid_ele.length==0) {
    ...
    if (item.getAttribute('data-status')=='Accept') {
